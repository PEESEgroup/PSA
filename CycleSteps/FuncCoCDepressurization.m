...
%#codegen
%CoCDepressurization: Calculate the change in state variables for CoC depres step
...
%   
%% Boundary Conditions
%   The boundary condtions for the CoC depressurization, corresponding to the
%   heavy product end of the column being closed and the the light product 
%   end of the column being opened. For the heavy product end, the following 
%   boundary condtions are used:
%   
%%  
%   $$ \frac{\partial \bar{P}}{\partial \tau} = 0 \quad | Z=0^- $$
%   
...
%   
%%  
%   $$ \frac{\partial \bar{P}}{\partial \tau} = \lambda(\bar{P_I}-\bar{P}) \quad | Z=0^- $$
%   
...
%%  Boundary Derivatives
    dPdt(1)    = dPdt(2)                     ;
    dPdt(N+2)  = tau*L/v_0.*(P_I/P_0-P(N+2)) ;
    dydt(1)    = dydt(2)                  ;
    dydt(N+2)  = dydt(N+1)                ;
    dx1dt(1)   = 0                        ;
    dx2dt(1)   = 0                        ;
